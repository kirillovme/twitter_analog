...
<form action="${path}" method="post">
      ...
      <div class="form-group row">
              ...
              <div class="col-sm-6">
                  <input type="email" value="<#if user??>${user.email}</#if>" name="email" class="form-control ${(emailError??)?string('is-invalid', '')}" placeholder="some@some.com" />
                  <#if emailError??>
                    <div class="invalid-feedback">
                        ${emailError}
                    </div>
      ...
      <div class="col-sm-6">
        <div class="g-recaptcha" data-sitekey="6LfaAtoUAAAAAOmmfoDjKcVDA2VvuTHskRk5B-8Y"></div>
           <#if captchaError??>
